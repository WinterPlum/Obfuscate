...
public class MySetupApplication {
    ...
    while (hasChanged) {
      ...
      for (Entry<String, Set<SootMethodAndClass>> entry : jimpleClass.getCallbackMethods()
        ...
        if (curCallbacks != null) {
          if (curCallbacks.addAll(entry.getValue())) hasChanged = true;
        } else {
   ...
   * @param jimpleClass
   *          The analysis class that gives us a mapping between
    ...
    for (Entry<String, Set<Integer>> lcentry : jimpleClass.getLayoutClasses().entrySet()) {
      ...
      for (Integer classId : lcentry.getValue()) {
        ...
        if (resource instanceof StringResource) {
          ...
          if (controls != null) for (LayoutControl lc : controls)
   ...
   *           Thrown if a required configuration cannot be read
   */
  private void calculateCallbackMethodsFast(ARSCFileParser resParser, LayoutFileParser lfp)
      throws IOException {
   ...
   * 
   * @param callbackClass
   *          The class with which to associate the layout
   *          callbacks
   * @param lc
  ...
  private void registerCallbackMethodsForView(SootClass callbackClass, LayoutControl lc) {
    ...
    for (SootClass parentClass : Scene.v().getActiveHierarchy().getSuperclassesOf(sc)) {
      if (parentClass.getName().startsWith("android."))
        ...
        if (!sm.isConstructor()) systemMethods.add(sm.getSubSignature());
    }

    // Scan for methods that overwrite parent class methods
    for (SootMethod sm : sc.getMethods())
      if (!sm.isConstructor()) if (systemMethods.contains(sm.getSubSignature()))
  ...
  public InfoflowResults runInfoflow(ResultsAvailableHandler onResultsAvailable) {
    ...
    System.out.println("Running data flow analysis on " + apkFileLocation + " with "
        + getSources().size() + " sources and " + getSinks().size() + " sinks...");
